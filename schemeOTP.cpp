...
messages per cipherfile must narrow.  Proof:   github.com/compromise-evident/OTP
--------------------------------------------------------------------------------
...
{	ifstream in_stream;
	...
	{	ifstream in_stream;
		...
		//Overwrites the used key file before removing it.
		out_stream.open(file_name_key_outgoing);
		for(int a = 0; a < 2000014; a++) {out_stream << '\0';} //Binary: 00000000
		out_stream.close();
		
		out_stream.open(file_name_key_outgoing);
		for(int a = 0; a < 2000014; a++) {out_stream.put(-1);} //Binary: 11111111 (same as 255 - extended ASCII.)
		out_stream.close();
